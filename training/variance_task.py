...
class VarianceTask(BaseTask):
    ...
    def build_losses_and_metrics(self):
        ...
        if self.train_tpse:
            ...
            if self.train_me_tpse:
                self.me_tpse_loss = build_tpse_loss()
                self.register_validation_loss('me_tpse_loss')
    ...
    def run_model(self, sample, infer=False):
        ...
        dur_pred, pitch_pred, variances_pred, tpse_pred, gst_pred, me_tpse_pred, me_gst_pred = output
        if infer:
            if dur_pred is not None:
                dur_pred = dur_pred.round().long()
            return dur_pred, pitch_pred, variances_pred, tpse_pred, gst_pred, me_tpse_pred, me_gst_pred  # Tensor, Tensor, Dict[str, Tensor]
        else:
            losses = {}
            if dur_pred is not None:
    ...
    def _validation_step(self, sample, batch_idx):
        ...
        if min(sample['indices']) < hparams['num_valid_plots']:
            ...
            dur_preds, pitch_preds, variances_preds, tpse_pred, gst_pred, me_tpse_pred, me_gst_pred = self.run_model(sample, infer=True)
